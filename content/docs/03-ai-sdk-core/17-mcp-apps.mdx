...
- Treat MCP App HTML as untrusted content. Web hosts must render it through a sandbox proxy on a dedicated origin that has no host cookies, storage, service worker, authenticated routes, or application data.
- Grant browser capabilities with `sandbox.allowedPermissions` only after host policy and, where appropriate, user consent. Resource metadata merely requests permission and grants nothing by itself.
