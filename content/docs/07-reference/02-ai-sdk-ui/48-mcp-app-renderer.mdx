...
const sandboxUrl = process.env.NEXT_PUBLIC_MCP_APP_SANDBOX_URL;
if (sandboxUrl == null) {
  throw new Error('NEXT_PUBLIC_MCP_APP_SANDBOX_URL is required');
}
  ...
  content={[
    {
      ...
      type: 'string | URL',
      description:
        'The URL of the sandbox proxy iframe. In web hosts it must use a dedicated origin with no access to host cookies, storage, service workers, authenticated routes, or application data.',
    },
    {
      name: 'title',
      type: 'string',
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Expected sandbox proxy origin after a known redirect. By default, the origin is derived from `url`. Wildcard, malformed, and opaque origins are rejected.',
    },
    {
      name: 'outerSandbox',
      type: 'string',
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Browser capabilities the host may grant after intersecting them with valid resource requests. Omitting this deny-by-default allowlist grants no camera, microphone, geolocation, or clipboard-write access.',
    },
  ]}
/>
  ...
  content={[
    {
      ...
      isOptional: true,
      description:
        'Client-side allowlist checked before forwarding `tools/call` requests. Omitting it exposes no tools.',
    },
    {
      name: 'callTool',
    ...
    {
      name: 'listResources',
      type: '(params?: { cursor?: string }) => Promise<unknown> | unknown',
      isOptional: true,
      description:
        'Handles app-initiated `resources/list` requests. Results are filtered to `ui://` resources before they reach the app.',
    },
    {
      name: 'openLink',
    ...
    {
      name: 'sendMessage',
      type: "(params: { role: 'user'; content: MCPAppContentBlock[] }) => Promise<unknown> | unknown",
      isOptional: true,
      description: 'Handles app-initiated `ui/message` requests.',
    },
    {
      name: 'updateModelContext',
      type: '(params: { content?: MCPAppContentBlock[]; structuredContent?: JSONObject }) => Promise<unknown> | unknown',
      isOptional: true,
