...
- **webSocket** _WebSocketConstructor | WebSocketFactory_

  Custom WebSocket implementation used by streaming transcription and the
  Responses API WebSocket transport. By default, the provider uses the runtime
  global `WebSocket`. You can pass a factory function to adapt a WebSocket
  client with a different constructor signature.
...
OpenAI's [WebSocket mode](https://developers.openai.com/api/docs/guides/websocket-mode)
reduces latency between model steps in tool-calling workflows. Enable it with
the OpenAI-specific `transport` provider option:
...
`generateText` supports the same option. It buffers the WebSocket event stream
and returns the same result shape as an HTTP request.

For ordinary tool loops, the provider reuses the socket when the next model
step contains the matching client-executed tool results. A response with no
pending client tool calls closes the socket immediately. Like AI SDK tool
execution itself, the provider does not impose an additional timeout while
waiting for matching tool results. Explicit aborts, stream cancellation, and
remote socket closure still close the connection. Unrelated calls do not use a
general connection pool.

<Note>
  WebSocket transport is only available for OpenAI Responses models. Chat,
  completion, embedding, image, and other model calls continue to use HTTP.
  WebSocket responses do not include HTTP response headers.
</Note>

The provider uses the runtime global `WebSocket` by default. Current Node.js,
Bun, and Deno runtimes expose the connection-header extension needed for OpenAI
authentication. If your runtime does not, or if you want to use another client,
adapt a library such as [`ws`](https://www.npmjs.com/package/ws):
...
Pass an arrow function for the factory form. Regular constructible functions
continue to use the existing `WebSocketConstructor` behavior for backward
compatibility.

The same `webSocket` setting is also used for OpenAI streaming transcription.
