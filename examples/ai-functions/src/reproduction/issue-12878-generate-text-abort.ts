...
async function main() {
  ...
  const model = new MockLanguageModelV3({
    doGenerate: async ({ abortSignal }) => {
      ...
      if (!abortSignal?.aborted) {
        throw new Error(
          'Reproduction setup failed: second model call was not aborted',
        );
      }
  ...
  try {
    ...
    if (
      !observed.signalAborted ||
      observed.steps !== 2 ||
      observed.modelCallCount !== 2
    ) {
      throw new Error(
        `Reproduction setup failed: unexpected result ${JSON.stringify(observed)}`,
      );
    }
